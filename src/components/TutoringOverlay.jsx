...
const TutoringOverlay = ({ progress }) => {
  return (
    <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[10000] flex flex-col items-center justify-center p-6 text-center">
      ...
      <h2 className="text-6xl font-black text-white mb-4 uppercase italic tracking-tighter">Đang dạy gia sư...</h2>
      <p className="text-slate-400 text-2xl mb-16 max-w-2xl">Hãy giảng bài thật tận tâm, thời gian trôi qua hơi chậm đấy!</p>

      <div className="w-full max-w-3xl bg-slate-800 h-10 rounded-full overflow-hidden border-4 border-white/10 shadow-2xl">
        <div 
          className="h-full bg-gradient-to-r from-indigo-600 via-purple-500 to-indigo-400 transition-all duration-100 ease-linear flex items-center justify-end px-5"
...
export default TutoringOverlay;
